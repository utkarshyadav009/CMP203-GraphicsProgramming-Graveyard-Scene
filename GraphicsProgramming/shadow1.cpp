...
void shadow1::generateShadowMatrix(float* shadowMatrix, float light_pos[4], GLfloat floor[12]) {
...
std::vector<float> shadow1::buildShadowVolume(float lightPosit[4], std::vector<float> verts)
{
...
void shadow1::extendVertex(float newVert[3], float lightPosit[4], float x, float y, float z, float ext)
{
	float lightDir[3];
